...
exp = design.Experiment(name = "Disrupting the causal perception via a spatial gap") # Set up experiment
...
# It appears that perceived causality returns when the red square stops at -55 pixels from the centre
while square2.position[0] < -55: # As long as the red square is not juxtaposed with the green square, do as follows:
...
while square1.position[0] < 400: # As long as the green square is not equidistant with the red square's starting position, do as follows:
    square1.move(offset=(5,0)) # Move the green square 5 pixels to the right
